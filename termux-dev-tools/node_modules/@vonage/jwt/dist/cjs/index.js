...
var log2 = (0, import_debug2.default)("vonage:jwt:verifySignature");
var verifySignature = (jwt2, privateKey) => {
  if (!privateKey) {
    throw new MissingPrivateKeyError();
  }
  if (typeof privateKey !== "string" && !(privateKey instanceof Buffer)) {
  ...
  try {
    verify(jwt2, privateKey, {
      algorithms: ["RS256", "HS256"]
    });
    return true;
  } catch (error) {
    log2("Error when verifying token", error);
  }
  return false;
};
...
var JWT = class {
   ...
   */
  verifySignature(jwt2, privateKey) {
    return verifySignature(jwt2, privateKey);
  }
  /**
